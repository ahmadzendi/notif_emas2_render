...
TELEGRAM_TOKEN = "8547890114:AAEYdlI6wUZWeMvSI0v4gE9xv_rxkiBlKLU"
TELEGRAM_CHAT_ID = "-1003506721905"
OANDA_ACCESS_TOKEN = "bb7c971b9cf64fd4808397f9243c48ae-355a30cebe0a7357b0dfa931566be3a0"
OANDA_ACCOUNT_ID = "101-004-38182636-001"
...
TIMEOUT_TREASURY = aiohttp.ClientTimeout(total=5, connect=2, sock_connect=2, sock_read=3)
TIMEOUT_SEND = aiohttp.ClientTimeout(total=5)
TIMEOUT_FAST = aiohttp.ClientTimeout(total=3)
TIMEOUT_SLOW = aiohttp.ClientTimeout(total=5)
...
async def fetch_treasury_with_retry(session):
    ...
    for attempt in range(3):
        ...
        print(f"Treasury fetch attempt {attempt + 1} gagal, retry dalam {backoff}s...", flush=True)
...
async def main_async():
    ...
    try:
        ...
        while True:
            ...
            if counter >= 100:
                counter = 0
                task = asyncio.create_task(get_telegram_updates(secondary_session))
                pending_tasks.add(task)
                task.add_done_callback(pending_tasks.discard)
